...
def main():
    ...
    # Filter: keep items with no CVE (rumors/0-day) OR items with any CVE not in KEV
    filtered = []
    for it in all_items:
    ...
    # Update state only when new items exist (to reduce commit noise)
    if new_items:
    ...
    print(f"[info] collected={len(all_items)}; after filter={len(final_items)}; new={len(new_items)}")
